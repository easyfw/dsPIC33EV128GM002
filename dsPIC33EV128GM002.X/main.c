 ...
 * Author: easyf
 *
 * Created on 2025년 8월 30일 (토), 오전 7:38
 */
/*
 * File:   main.c
 * Author: easyf
...
// (Configuration bits and other functions are the same as the previous answer)
...
#pragma config FWDTEN = OFF, ICS = PGD1, BOREN0 = OFF, ALTI2C1 = OFF
...
int ZSSC4151_Read_Command_Response32(uint8_t command, uint32_t *response_data)
{
    ...
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(command)) { I2C1_Stop(); return -1; }
    if (!I2C1_Restart()) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(ZSSC4151_READ_ADDR)) { I2C1_Stop(); return -1; }
...
int ZSSC4151_Clear_Status(void)
{
    ...
    if (!I2C1_Start()) return -1;
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CMD_CLEAR_STATUS)) { I2C1_Stop(); return -1; }
    
    // 32??? ????? ???? (??? ????? 1?? ??????? ??? ?????)
    if (!I2C1_Write(CLEAR_ALL_MSB)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CLEAR_ALL_LSB)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CLEAR_ALL_MSB)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CLEAR_ALL_LSB)) { I2C1_Stop(); return -1; }
...
int ZSSC4151_Write_NVM(unsigned char reg_addr, unsigned int data) 
{
    ...
    // NVRAM ?????? ?ð??? ??? ?? ??????? ????? ??????? ????.
    delay_10ms(2); // ?????????? t_NVM_prog ?ð??? ??????? ??????? 20ms ???
...
bool SHT4x_Read_TH(float *T, float *H)
{
    ...
    if (!I2C1_Write(SHT4X_WRITE_ADDR)) { I2C1_Stop(); return false; }
    if (!I2C1_Write(SHT4X_CMD_MEASURE_HPM)) { I2C1_Stop(); return false; }
    I2C1_Stop();
    ...
    if (!I2C1_Start()) return false;
    if (!I2C1_Write(SHT4X_READ_ADDR)) { I2C1_Stop(); return false; }
...
int ZSSC4151_EnterCommandMode(void) 
{
    if (!I2C1_Start()) return -1;
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CMD_START_CM)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CM_AUTH_KEY_MSB)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CM_AUTH_KEY_LSB)) { I2C1_Stop(); return -1; }
    if (!I2C1_Stop()) return -1;
...
int ZSSC4151_Start_Normal_Mode(void)
{
    if (!I2C1_Start()) return -1;
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CMD_START_NOM)) { I2C1_Stop(); return -1; }
    if (!I2C1_Stop()) return -1;
...
bool ZSSC4151_Read_Normal_Mode_Output(uint16_t* read_word)
{
    ...
    // 3. ?????κ??? 2????? ?????? ?б?
    rx_buffer[0] = I2C1_Read(true);  // MSB ?а?, ACK ????
    rx_buffer[1] = I2C1_Read(false); // LSB ?а?, NACK ????
...
/**
 * @brief Normal Mode???? ZSSC4151?? ??? RAM ???? ?н????. (burst mode ???)
 * @param ram_addr ?а? ???? RAM?? ???? ???
 * @param read_data ?о?? 16??? ??????? ????? ??????
 * @return 0: ????, -1: ????
 */
int ZSSC4151_Read_Output_Data(uint8_t ram_addr, uint16_t *read_data)
{
    ...
    if (!I2C1_Start()) return -1;
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CMD_READ_OUTPUT_MEM_BURST)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(arg_msb)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(arg_lsb)) { I2C1_Stop(); return -1; }
...
bool ZSSC4151_ReadNvm(uint8_t addr, uint16_t *val)
{
    ...
    // --- 1. ???? ????? ????? ?????? ???? ?????? ---
    if (!I2C1_Start()) return false;
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return false; }
    if (!I2C1_Write(command_with_addr))   { I2C1_Stop(); return false; }
    ...
    // --- 2. ??????? ???????? ?б? ?????? ---
    if (!I2C1_Write(ZSSC4151_READ_ADDR)) { I2C1_Stop(); return false; }
...
int ZSSC4151_Unlock_NVM(void) 
{
    ...
    if (!I2C1_Start()) return -1;
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CMD_WR_NVM_AUTH)) { I2C1_Stop(); return -1; }
    
    // 16??? ???? ? ????
    if (!I2C1_Write((uint8_t)(AUTH_KEY >> 8))) { I2C1_Stop(); return -1; }   // MSB
    if (!I2C1_Write((uint8_t)(AUTH_KEY & 0xFF))) { I2C1_Stop(); return -1; } // LSB
...
int ZSSC4151_ReadNvmWord(uint8_t nvm_addr, uint16_t *read_data)
{
    ...
    if (!I2C1_Start()) return -1;
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CMD_READ_NVM)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(0x00)) { I2C1_Stop(); return -1; } 
    if (!I2C1_Write(nvm_addr)) { I2C1_Stop(); return -1; }
...
int ZSSC4151_WriteNvmWord(uint8_t nvm_addr, uint16_t write_data)
{
    ...
    if (!I2C1_Start()) return -1;
    if (!I2C1_Write(ZSSC4151_WRITE_ADDR)) { I2C1_Stop(); return -1; }
    if (!I2C1_Write(CMD_WRITE_NVM)) { I2C1_Stop(); return -1; }
    ...
    // ?????? ????
    if (!I2C1_Write((uint8_t)(write_data >> 8))) { I2C1_Stop(); return -1; }
    if (!I2C1_Write((uint8_t)(write_data & 0xFF))) { I2C1_Stop(); return -1; }
